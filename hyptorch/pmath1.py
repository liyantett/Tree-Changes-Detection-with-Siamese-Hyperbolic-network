...
def _tensor_dot1(x, y):
    #print('_tensor_dot1')
   
    #print('x1',x.size())
    #print('x1',y.size())
    res1 = torch.einsum("ij,kj->ik", (x, y))
...
def _mobius_addition_batch1(x, y, c):
    xy = _tensor_dot1(x, y)  # B x C
...
def _hyperbolic_softmax(X, A, P, c):
    ...
    mob_add = _mobius_addition_batch1(-P, X, c)
...
def _dist_matrix1(x, y, c):
    sqrt_c = c ** 0.5
    return (
        2
        / sqrt_c
        * artanh(sqrt_c * torch.norm(_mobius_addition_batch1(-x, y, c=c), dim=-1))
    )


def dist_matrix1(x, y, c=1.0):
    c = torch.as_tensor(c).type_as(x)
    return _dist_matrix1(x, y, c)
